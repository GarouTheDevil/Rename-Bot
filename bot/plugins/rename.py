...
async def rename_handler(c: Client, m: Message):
    ...
    if m.from_user.id not in Config.PRO_USERS:
        ...
        if is_in_gap:
            await m.reply_text("No Dual Task\n"
                               f"Wait For : `{str(sleep_time)}s`",
                               quote=True)
            return
    await add_user_to_database(c, m)
    if (not m.reply_to_message) or (not m.reply_to_message.media) or (not get_file_attr(m.reply_to_message)):
        return await m.reply_text("**Reply To Media**", quote=True)

    # Proceed
    editable = await m.reply_text("**Send New File Name**", quote=True)
    ...
    try:
        ...
        if not file_id:
            return await editable.edit("**Process Failed**\n"
                                       "**Check The File**")
